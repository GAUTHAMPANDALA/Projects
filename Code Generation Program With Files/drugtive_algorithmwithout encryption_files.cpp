...

{
	...
	//printf("s is: %s\n",s);
    char s6[7]={'a'-41,'a'-42,'a'-40,'a'-41,'a'-40,'a'-42,'a'-41};													//creating that size of array string	
	...
	if (strlen(s)<14)
	{
		...
		//printf("s6 is: %s\n",s6);
	    strncat(s,s6,d1);
		strrev(s);												// concating the remaing places after the number  with zeros(before important)
	}
	//printf("%s\n",s);
	fprintf(p,"the unique code is: %s\n",s);
